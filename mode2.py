...
def render():
    ...
    _saved2 = (_db.get_strategy(_code, 2)
               if _code and _db.available() else None)
    _sel_idx = 0
    if _saved2 and _saved2.get("plan_name") in plans:
        _sel_idx = list(plans.keys()).index(_saved2["plan_name"])
        st.caption(f"📁 已从档案自动恢复你的策略：{_saved2['plan_name']}")
    sel = st.selectbox("选择你要执行的方案", list(plans.keys()),
                       index=_sel_idx, key="m2x_plan")
    w_sel = plans[sel]
    ...
    # ---------- 保存策略到档案 ----------
    if _code:
        if st.button("💾 把当前选定方案保存为我的策略（下次自动恢复）",
                     key="m2_save_strategy"):
            if _db.save_strategy(_code, 2, {"plan_name": sel,
                                            "pool": pool_choice,
    ...
    # ================= 档案：买入记账与投后体检（模块二） =================
    st.divider()
    _w_map2 = {t: float(w_sel[i]) for i, t in enumerate(tickers)
               if w_sel[i] >= 0.005}
    _tracking.render_archive(
        _code, mode=2, target_map=_w_map2, prices=prices, names=names,
        product_tickers=list(_w_map2.keys()), total_plan=total_amount,
        key_prefix="m2_arch")
