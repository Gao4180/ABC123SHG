...
def render():
    ...
    with st.form("goal_form", clear_on_submit=True):
        ...
        with c1:
            ...
            gname = st.text_input("目标名称", value=gname_preset)
        ...
        if st.form_submit_button("添加目标", type="primary"):
            st.session_state["goals"].append({
                "name": gname, "years": g_years, "target": g_target,
                "start": g_start, "monthly": g_monthly})
            st.rerun()
    ...
    summary = []
    for g in goals:
        eq_share = min(glide_equity_share(g["years"]), eq_cap)
        ...
        with st.container(border=True):
            ...
            c3.metric("股票占比（下滑轨道）", f"{eq_share:.0%}")
            ...
            with col_r:
                st.plotly_chart(
                    ...
                    width="stretch", key=f"goal_fan_{g['name']}_{g['years']}")
            if prob < 0.6:
                st.warning("达成概率偏低：可考虑延长期限、提高每月定投、或调低目标金额。")
            elif prob >= 0.85:
                st.success("达成概率很高：也可以考虑降低股票占比，用更稳的方式到达。")
        summary.append({"目标": g["name"], "期限(年)": g["years"],
